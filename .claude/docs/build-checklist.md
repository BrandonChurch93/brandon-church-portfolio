...
- 2026-07-29 · **P10.2 · 🟡 PUSHED, BUT NOT DEPLOYED.** On Brandon's go, `main` was pushed to `origin`. **The push succeeded and is verified**: `git ls-remote origin main` returns `38e967d33fd72f6284458f05aad880ed0b90f156`, identical to local HEAD. Four commits landed: baseline, checkpoint, refinement, ship log.
- 2026-07-29 · **P10.2 · ⛔ The deploy did not fire. Production is still serving the pre-restructure build.** Polled for ~5 minutes with no change, then diagnosed rather than waiting longer. Evidence, all consistent:
  - `/work` returns **404** in production, and the production `sitemap.xml` has no `/work` entry and still lists projects in the **pre-P3 order** (modern-softworks first).
  - The production hero eyebrow still reads **"Design Engineer"** and a **"Featured Project"** eyebrow is still present, both replaced during this work.
  - Response headers show `server: Vercel`, `x-vercel-cache: HIT` and **`age: 153792`**, roughly **43 hours old**. A new deployment would have invalidated that.
  - The repo contains **no `.vercel` directory and no `vercel.json`**, so nothing links this working copy to a Vercel project.
  - The Vercel CLI is installed and authenticated as **`modernsoftworks-2766`**.
  - Conclusion: the code is on GitHub, but **Vercel's GitHub integration is not building this repo's `main`**, or the production project is connected to a different repo/branch. This is account-side configuration, not a code problem, and the build is green locally.
  - Deliberately not attempted: `vercel ls`/`vercel link` in this unlinked directory would create a project link as a side effect, and `vercel deploy` would push a build straight from local, bypassing git and risking a deployment that does not match `origin/main`. Both are Brandon's calls.
- 2026-07-29 · **P10.3 · ⏸️ STILL BLOCKED.** The post-deploy production spot check cannot run until a build from `38e967d` is actually live.
