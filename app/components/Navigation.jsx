...
const Navigation = () => {
  ...
  const navItems = [
    ...
    { label: "Design System", href: "/design-system", isPage: true },
  ];
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = navItems.filter((item) => !item.isPage).map((item) => item.href.substring(1));
              ...
              <ul className="flex items-center gap-6">
                {navItems.map((item) => (
                  <li key={item.label}>
                    <a
                      href={item.href}
                      onClick={item.isPage ? undefined : (e) => scrollToSection(e, item.href)}
                      className={cn(
                    ...
                    >
                      {item.label}
                      {!item.isPage && activeSection === item.href.substring(1) && (
                        <motion.div
